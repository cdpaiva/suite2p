...
from conftest import (
    initialize_ops,
    download_cached_inputs,
)  # Guarantees that tests and this script use the same ops
from tests.regression.utils import (
    FullPipelineTestUtils,
    DetectionTestUtils,
    ExtractionTestUtils,
)
...
class GenerateFullPipelineTestData:
    ...
    def generate_1p1c1500_expected_data(ops):
        ...
        """
        test_ops = FullPipelineTestUtils.initialize_ops_test1plane_1chan_with_batches(
            ops.copy()
        )
    ...
    def generate_2p2c1500_expected_data(ops):
        ...
        """
        test_ops = FullPipelineTestUtils.initialize_ops_test2plane_2chan_with_batches(
            ops.copy()
        )
        suite2p.run_s2p(ops=test_ops)
    ...
    def generate_2p2zmesoscan_expected_data(ops):
        ...
        """
        test_ops = FullPipelineTestUtils.initialize_ops_test_mesoscan_2plane_2z(
            ops.copy()
        )
        suite2p.run_s2p(ops=test_ops)
...
class GenerateDetectionTestData:
    ...
    def generate_detection_1plane1chan_test_data(ops):
        ...
        with suite2p.io.BinaryFile(
            Ly=ops[0]["Ly"], Lx=ops[0]["Lx"], filename=ops[0]["reg_file"]
        ) as f_reg:
    ...
    def generate_detection_2plane2chan_test_data(ops):
        ...
        for i in range(len(two_plane_ops)):
            ...
            with suite2p.io.BinaryFile(
                ...
                np.save(
                    "expected_detect_output_%ip%ic%i.npy"
                    % (ops["nchannels"], ops["nplanes"], i),
                    output_dict,
                )
    ...
    def generate_all_data(ops):
        ...
        shutil.move(
            "expected_detect_output_1p1c0.npy", test_data_dir_path.joinpath("detection")
        )
        shutil.move(
            "expected_detect_output_2p2c0.npy", test_data_dir_path.joinpath("detection")
        )
        shutil.move(
            "expected_detect_output_2p2c1.npy", test_data_dir_path.joinpath("detection")
        )
...
class GenerateClassificationTestData:
    ...
    def generate_classification_test_data(ops):
        stat = np.load(
            test_input_dir_path.joinpath("test_inputs/classification/pre_stat.npy"),
            allow_pickle=True,
        )
        iscell = suite2p.classification.classify(
            stat, classfile=suite2p.classification.builtin_classfile
        )
        np.save(
            str(
                test_data_dir_path.joinpath("classification").joinpath(
                    "expected_classify_output_1p1c0.npy"
                )
            ),
            iscell,
        )
...
class GenerateExtractionTestData:
    ...
    def generate_preprocess_baseline_test_data(ops):
        ...
        for bv in baseline_vals:
            ...
            np.save(
                str(test_data_dir_path.joinpath("extraction/{}_f.npy".format(bv))),
                pre_f,
            )
    ...
    def generate_extraction_output_1plane1chan(ops):
        ...
        extract_input = np.load(
            str(
                test_data_dir_path.joinpath(
                    "detection/expected_detect_output_1p1c0.npy"
                )
            ),
            allow_pickle=True,
        )[()]
        ...
        remove_binary_file(test_data_dir_path, 0, "")
        os.rename(
            os.path.join(test_data_dir_path, "suite2p"),
            os.path.join(test_data_dir_path, "1plane1chan"),
        )
        shutil.move(
            os.path.join(test_data_dir_path, "1plane1chan"),
            os.path.join(test_data_dir_path, "extraction"),
        )
    ...
    def generate_extraction_output_2plane2chan(ops):
        ...
        ops = ExtractionTestUtils.prepare(
            ...
            [
                [
                    Path(ops["data_path"][0]).joinpath(
                        "detection/pre_registered01.npy"
                    ),
                    Path(ops["data_path"][0]).joinpath(
                        "detection/pre_registered02.npy"
                    ),
                ],
                [
                    Path(ops["data_path"][0]).joinpath(
                        "detection/pre_registered11.npy"
                    ),
                    Path(ops["data_path"][0]).joinpath(
                        "detection/pre_registered12.npy"
                    ),
                ],
            ],
            (404, 360),
        ...
        ops[0]["meanImg_chan2"] = np.load(
            Path(ops[0]["data_path"][0]).joinpath("detection/meanImg_chan2p0.npy")
        )
        ops[1]["meanImg_chan2"] = np.load(
            Path(ops[1]["data_path"][0]).joinpath("detection/meanImg_chan2p1.npy")
        )
        ...
        extract_inputs = [
            np.load(
                str(
                    test_data_dir_path.joinpath(
                        "detection/expected_detect_output_2p2c0.npy"
                    )
                ),
                allow_pickle=True,
            )[()],
            np.load(
                str(
                    test_data_dir_path.joinpath(
                        "detection/expected_detect_output_2p2c1.npy"
                    )
                ),
                allow_pickle=True,
            )[()],
        ...
        os.rename(
            os.path.join(test_data_dir_path, "suite2p"),
            os.path.join(test_data_dir_path, "2plane2chan"),
        )
        shutil.move(
            os.path.join(test_data_dir_path, "2plane2chan"),
            os.path.join(test_data_dir_path, "extraction"),
...
def remove_binary_file(dir_path, plane_num, bin_file_suffix):
    os.remove(
        os.path.join(
            dir_path, "suite2p/plane{}/data{}.bin".format(plane_num, bin_file_suffix)
        )
    )
