...
def prepare_for_registration(op, input_file_name, dimensions):
    ...
    input_data = (imread(str(input_file_name)) // 2).astype(np.int16)
    ...
    for i in range(op["nplanes"]):
        ...
        chan1_end = (
            plane_start + nframes_per_plane // nc
            if nc == 2
            else nframes_per_plane * (i + 1)
        )
        curr_op = op.copy()
        bin_file_path = str(
            Path(curr_op["save_path0"]).joinpath("data_{}.bin".format(i))
        )
        ...
        if nc == 2:
            bin_file_path2 = str(
                Path(curr_op["save_path0"]).joinpath("data_{}_2.bin".format(i))
            )
...
def check_registration_output(
    op, dimensions, input_path, reg_output_path_list, output_path_list
):
    ops = prepare_for_registration(op, input_path, dimensions)
    reg_ops = []
    npl = op["nplanes"]
    for i in range(npl):
        # curr_op = register_binary(ops[i])
        raw = (
            ops[i].get("keep_movie_raw")
            and "raw_file" in ops[i]
            and os.path.isfile(ops[i]["raw_file"])
        )
        ...
        if ops[i]["nchannels"] > 1:
            ...
            raw_file_chan2 = ops[i].get("raw_file_chan2", 0) if raw else reg_file_chan2
        else:
        ...
        with (
            io.BinaryFile(Ly=Ly, Lx=Lx, filename=raw_file) if raw else null
        ) as f_raw, io.BinaryFile(Ly=Ly, Lx=Lx, filename=reg_file) as f_reg, (
            io.BinaryFile(Ly=Ly, Lx=Lx, filename=raw_file_chan2)
            if raw and twoc
            else null
        ) as f_raw_chan2, (
            io.BinaryFile(Ly=Ly, Lx=Lx, filename=reg_file_chan2) if twoc else null
        ) as f_reg_chan2:
