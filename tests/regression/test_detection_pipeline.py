...
def detect_wrapper(ops):
    ...
    for i in range(len(ops)):
        ...
        output_check = np.load(
            op["data_path"][0].parent.joinpath(
                f"test_outputs/detection/expected_detect_output_{ op['nplanes'] }p{ op['nchannels'] }c{ i }.npy"
            ),
            allow_pickle=True,
        )[()]
        # assert np.array_equal(output_check['cell_pix'], cell_pix)
        cell_masks, neuropil_masks = masks.create_masks(
            stat, op["Ly"], op["Lx"], ops=op
        )
        assert all(
            np.allclose(a, b, rtol=1e-4, atol=5e-2)
            for a, b in zip(cell_masks, output_check["cell_masks"])
        )
        assert all(
            np.allclose(a, b, rtol=1e-4, atol=5e-2)
            for a, b in zip(neuropil_masks, output_check["neuropil_masks"])
        )
            ...
            utils.compare_list_of_outputs(
                ...
                utils.get_list_of_data(
                    ...
                    test_ops["data_path"][0].parent.joinpath(
                        f"test_outputs/detection/suite2p/plane{i}"
                    ),
                ),
                utils.get_list_of_data(
                    outputs_to_check,
