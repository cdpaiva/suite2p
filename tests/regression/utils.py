    ...
    for output in outputs_to_check:
        ...
        if "reg_tif" in output:
            yield np.concatenate(
                [imread(tif) for tif in glob(str(data_path.joinpath("*.tif")))]
            )
        else:
            yield np.load(str(data_path) + ".npy", allow_pickle=True)


def compare_list_of_outputs(
    output_name_list, data_list_one, data_list_two
) -> Iterator[bool]:
    ...
    def prepare(op, input_file_name_list, dimensions):
        ...
        for plane in range(op["nplanes"]):
            ...
            BinaryFile.convert_numpy_file_to_suite2p_binary(
                str(input_file_name_list[plane][0]), bin_path
            )
            ...
            if curr_op["nchannels"] == 2:
                ...
                BinaryFile.convert_numpy_file_to_suite2p_binary(
                    str(input_file_name_list[plane][1]), bin2_path
                )
