    ...
    for plane in range(ops[0]["nplanes"]):
        ...
        extract_input = np.load(
            curr_op["data_path"][0].parent.joinpath(
                "test_outputs",
                "detection",
                "expected_detect_output_{0}p{1}c{2}.npy".format(
                    curr_op["nplanes"], curr_op["nchannels"], plane
                ),
            ),
            allow_pickle=True,
        )[()]
        ...
        spks = extraction.oasis(
            F=dF, batch_size=curr_op["batch_size"], tau=curr_op["tau"], fs=curr_op["fs"]
        )
    ...
    for bv in baseline_vals:
        ...
        test_f = np.load(
            test_ops["data_path"][0].parent.joinpath(
                "test_outputs/extraction/{}_f.npy".format(bv)
            )
        )
...
def test_pre_process_baseline(test_ops):
    f = np.load(
        test_ops["data_path"][0].parent.joinpath(
            "test_outputs/1plane1chan1500/suite2p/plane0/F.npy"
        )
    )
        ...
        assert all(
            utils.compare_list_of_outputs(
                ...
                utils.get_list_of_data(
                    outputs_to_check,
                    Path(ops["data_path"][0]).parent.joinpath(
                        f"test_outputs/extraction/1plane1chan/plane0"
                    ),
                ),
                utils.get_list_of_data(
                    outputs_to_check,
                    Path(ops["save_path0"]).joinpath(f"suite2p/plane0"),
                ),
            )
        )
        ...
        assert all(
            utils.compare_list_of_outputs(
                ...
                utils.get_list_of_data(
                    outputs_to_check,
                    Path(
                        ops[i]["data_path"][0].parent.joinpath(
                            f"test_outputs/extraction/2plane2chan/plane{i}"
                        )
                    ),
                ),
                utils.get_list_of_data(
                    outputs_to_check,
                    Path(ops[i]["save_path0"]).joinpath(f"suite2p/plane{i}"),
                ),
            )
        )
