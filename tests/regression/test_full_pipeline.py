...
def test_1plane_1chan_with_batches_metrics_and_exported_to_nwb_format(test_ops):
    ...
    """
    test_ops = utils.FullPipelineTestUtils.initialize_ops_test1plane_1chan_with_batches(
        test_ops
    )
        ...
        assert all(
            utils.compare_list_of_outputs(
                ...
                utils.get_list_of_data(
                    outputs_to_check,
                    test_ops["data_path"][0].parent.joinpath(
                        f"test_outputs/{nplanes}plane{test_ops['nchannels']}chan1500/suite2p/plane{i}"
                    ),
                ),
                utils.get_list_of_data(
                    outputs_to_check,
                    Path(test_ops["save_path0"]).joinpath(f"suite2p/plane{i}"),
                ),
            )
        )
    # Read Nwb data and make sure it's identical to output data
    stat, ops, F, Fneu, spks, iscell, probcell, redcell, probredcell = io.read_nwb(
        str(Path(test_ops["save_path0"]).joinpath("suite2p/ophys.nwb"))
    )
    ...
    for output, data1, data2 in zip(output_name_list, data_list_one, data_list_two):
        if (
            output == "stat"
        ):  # where the elements of npy arrays are dictionaries (e.g: stat.npy)
            for gt_dict, output_dict in zip(data1, data2):
                for k in gt_dict.keys():
                    if (
                        k == "ypix" or k == "xpix"
                    ):  # todo: these both are different from the original; footprint and overlap are different, std key doesn't exist in output_dict.
                        assert np.allclose(
                            gt_dict[k], output_dict[k], rtol=1e-4, atol=5e-2
                        )
        elif (
            output == "iscell"
        ):  # just check the first column; are cells/noncells classified the same way?
...
def test_2plane_2chan_with_batches(test_ops):
    ...
    """
    for _ in range(2):
        ops = utils.FullPipelineTestUtils.initialize_ops_test2plane_2chan_with_batches(
            test_ops
        )
        nplanes = ops["nplanes"]
        ...
        for i in range(nplanes):
            assert all(
                utils.compare_list_of_outputs(
                    ...
                    utils.get_list_of_data(
                        outputs_to_check,
                        ops["data_path"][0].parent.joinpath(
                            f"test_outputs/{nplanes}plane{ops['nchannels']}chan1500/suite2p/plane{i}"
                        ),
                    ),
                    utils.get_list_of_data(
                        outputs_to_check,
                        Path(ops["save_path0"]).joinpath(f"suite2p/plane{i}"),
                    ),
                )
            )
        ...
        assert all(
            utils.compare_list_of_outputs(
                ...
                utils.get_list_of_data(
                    outputs_to_check,
                    test_ops["data_path"][0].parent.joinpath(
                        f"test_outputs/{nplanes}plane{test_ops['nchannels']}chan/suite2p/plane{i}"
                    ),
                ),
                utils.get_list_of_data(
                    outputs_to_check,
...
def test_mesoscan_2plane_2z(test_ops):
    ...
    """
    test_ops = utils.FullPipelineTestUtils.initialize_ops_test_mesoscan_2plane_2z(
        test_ops
    )
    suite2p.run_s2p(ops=test_ops)
    ...
    for i in range(nplanes):
        assert all(
            utils.compare_list_of_outputs(
                ...
                utils.get_list_of_data(
                    outputs_to_check,
                    test_ops["data_path"][0].parent.parent.joinpath(
                        f"test_outputs/mesoscan/suite2p/plane{i}"
                    ),
                ),
                utils.get_list_of_data(
                    outputs_to_check,
