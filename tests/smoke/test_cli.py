...
def test_cli_reg_metrics_runs_with_output(capfd, tmpdir):
    os.system(
        f"reg_metrics data/test_data/ --tiff_list input_1500.tif --do_registration 2 --save_path0 {tmpdir}"
    )
    captured = capfd.readouterr()
    assert captured.out
...
def temp_test_cli_suite2p_gui_runs_when_is_called_locally():
    s2p = subprocess.Popen(
        ["python", "-m", "suite2p"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    sleep(2)
