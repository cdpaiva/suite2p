...
def test_do_registration_do_roi_detect_settings_check_timing(test_ops):
    ...
    test_ops.update({"do_registration": True})
    reg_only_ops = suite2p.run_s2p(ops=test_ops)  # registration only
    assert list(reg_only_ops["timing"].keys()) == [
        "registration",
        "total_plane_runtime",
    ]
    test_ops.update(
    ...
    assert list(det_only_ops["timing"].keys()) == [
        "detection",
        "extraction",
        "classification",
        "total_plane_runtime",
    ]
    test_ops.update({"spikedetect": True})
    det_dec_ops = suite2p.run_s2p(ops=test_ops)  # detection & deconvolution
    assert list(det_dec_ops["timing"].keys()) == [
        "detection",
        "extraction",
        "classification",
        "deconvolution",
