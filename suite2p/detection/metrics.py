...
def match_func_anat(stat_func, stat_anat, Ly, Lx, threshold=0.5):
    ...
    for i, sf in enumerate(stat_func):
        ...
        inds = (
            slice(max(0, ymed - ly), min(ymed + ly, Ly)),
        ...
        for j, sa in enumerate(stat_anat):
            ...
            if (
                np.logical_and(ypix_a > inds[0].start, ypix_a < inds[0].stop).sum() > 0
                and np.logical_and(xpix_a > inds[1].start, xpix_a < inds[1].stop).sum()
                > 0
            ):
                lam_a = sa["lam"].copy()
...
def extend_anatomical(img_anat, masks_anat, mov=None, ops=None, reg_file=None):
    if mov is None:
        ...
        print(
            "Binned movie [%d,%d,%d] in %0.2f sec."
    ...
    if ops is not None:
        ...
        mov = neuropil_subtraction(
            mov=mov / sdmov, filter_size=ops["spatial_hp_detect"]
    ...
    for i in range(masks_anat.max()):
        ...
        inds = (
            slice(max(0, ymed - ly), min(ymed + ly, Lyc)),
        ...
        iou = _intersection_over_union(
            (maskb > 0).astype(np.uint16), output.astype(np.uint16)
        )[1, 1:]
