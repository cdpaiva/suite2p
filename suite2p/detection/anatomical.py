    ...
    for j in np.arange(0, npatches, batch_size):
        ...
        for i, yi in enumerate(y):
            ...
            maski = dynamics.get_masks(
                p, iscell=(cellprob > 0), flows=dP, threshold=1.0
            )
            maski = fill_holes_and_remove_small_masks(maski)
            maski = transforms.resize_image(
                maski, ly, ly, interpolation=cv2.INTER_NEAREST
            )
            pmasks[j + i] = maski
        if j % 5 == 0:
            print(
                "%d / %d masks created in %0.2fs"
...
def refine_masks(stats, patches, seeds, diam, Lyc, Lxc):
    ...
    for i, (patch_mask, stat, (yi, xi)) in enumerate(zip(patch_masks, stats, seeds)):
        ...
        ious = utils.mask_ious(
            patch_mask.astype(np.uint16), (func_mask > 0).astype(np.uint16)
        )[0]
...
def roi_detect(
    ...
    model = CellposeModel(
        pretrained_model=pretrained_model, gpu=True if core.use_gpu() else False
    )
    masks = model.eval(
        mproj,
        diameter=diameter,
