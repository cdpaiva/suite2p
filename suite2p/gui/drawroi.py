...
class ROIDraw(QMainWindow):
    ...
    def proc_ROI(self):
        ...
        if "reg_file_chan2" in self.parent.ops and not os.path.isfile(
            self.parent.ops["reg_file_chan2"]
        ):
            self.parent.ops["reg_file_chan2"] = os.path.join(
                self.parent.basename, "data_chan2.bin"
            )
...
class sROI:
    ...
    def rotate_ROI(self, parent, ellipse, xrange, yrange, posx, posy):
        ...
        xrange = (np.arange(-1 * int(ellipse.shape[1] - 1), 1) + int(posx)).astype(
            np.int32
        )
        yrange = (np.arange(-1 * int(ellipse.shape[0] - 1), 1) + int(posy)).astype(
            np.int32
        )
    ...
    def position(self, parent):
        ...
        if self.ROI.angle() not in (0, 180, -180):
            ellipse, xrange, yrange = self.rotate_ROI(
                parent, ellipse, xrange, yrange, posx, posy
            )
