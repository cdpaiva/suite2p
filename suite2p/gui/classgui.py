...
def class_default(parent):
    ...
    if dm == QMessageBox.Yes:
        ...
        save_model(
            classfile, parent.model.stats, parent.model.iscell, parent.model.keys
        )
...
def add_to(parent):
    ...
    if dm == QMessageBox.Yes:
        ...
        save_model(
            parent.classfile, parent.model.stats, parent.model.iscell, parent.model.keys
        )
        activate(parent, True)
        msg = QMessageBox.information(
            parent,
...
class ListChooser(QDialog):
    ...
    def load_cell(self):
        name = QFileDialog.getOpenFileName(
            self, "Open iscell.npy file", filter="iscell.npy"
        )
        if name:
            try:
    ...
    def load_text(self):
        name = QFileDialog.getOpenFileName(
            self, "Open *.txt file", filter="text file (*.txt)"
        )
        if name:
            try:
