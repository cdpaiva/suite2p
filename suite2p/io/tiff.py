        ...
        try:
            with ScanImageTiffReader(tiff_filename) as tif:
                (
                    tif.data()
                    if len(tif.shape()) < 3
                    else tif.data(beg=0, end=np.minimum(batch_size, tif.shape()[0] - 1))
                )
            return False
        except:
            print(
...
def ome_to_binary(ops):
    ...
    for ops1_0 in ops1:
        ...
        ops1_0["frames_per_file"] = (
            np.ones(len(fs_Ch1), "int")
            if n_pages == 1
            else np.zeros(len(fs_Ch1), "int")
        )
