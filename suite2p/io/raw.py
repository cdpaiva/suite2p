...
def raw_to_binary(ops, use_recorded_defaults=True):
    ...
    if not HAS_XML:
        raise ImportError(
            "xmltodict is required for RAW file support (pip install xmltodict)"
        )
    ...
    ops_paths = _initialize_destination_files(
        ops, raw_file_configurations, use_recorded_defaults=use_recorded_defaults
    )
...
def _initialize_destination_files(
    ops, raw_file_configurations, use_recorded_defaults=True
):
    ...
    assert all(
        conf == configurations[0] for conf in configurations
...
def _raw2bin(all_ops, cfg):
    ...
    with open(cfg.path, "rb") as raw_file:
        chunk = (
            frames_in_chunk * cfg.xpx * cfg.ypx * cfg.channel * cfg.recorded_planes * 2
        )
        ...
        while raw_data_chunk:
            ...
            if cfg.channel > 1:
                ...
                reshaped_data = data.reshape(
                    cfg.recorded_planes, current_frames, cfg.xpx, cfg.ypx
                )
            ...
            for plane in range(0, cfg.zplanes):
                ...
                if cfg.channel > 1:
                    ...
                    ops["meanImg_chan2"] = ops["meanImg_chan2"] + plane_data[1].astype(
                        np.float32
                    ).sum(axis=0)
                    ...
                    ops["meanImg"] = ops["meanImg"] + plane_data.astype(np.float32).sum(
                        axis=0
                    )
    ...
    for ops in all_ops:
        total_frames = int(
            cfg.size / cfg.xpx / cfg.ypx / cfg.recorded_planes / cfg.channel / 2
        )
...
class _RawFile(_RawConfig):
    ...
    def __init__(self, dir_name):
        ...
        # Find main raw file
        main_files = [
            fn for fn in filenames if fn.lower().endswith(self._MAIN_FILE_SUFFIX)
        ]
        ...
        assert 1 == len(
            xml_files
        ), f'Missing required XML configuration file from dir="{dir_name}"'
